<script lang="ts">
    ...
    import { CloseOutline, FilePdfOutline, FilterOutline, PlusOutline, SortOutline, TableColumnOutline } from 'flowbite-svelte-icons';
                        ...
                        <TableBody>
                            {#each inspectors as inspector}
                                <TableBodyRow on:click={() => handleRowClick(inspector.id)} class="cursor-pointer">
                                    {#each userActiveHeaders as header}
                                        <TableBodyCell class="ps-4 text-center">{mapInspectorData(inspector, userActiveHeaders)[header]}</TableBodyCell>
                                    {/each}
                                </TableBodyRow>
                            {/each}
                        </TableBody>
